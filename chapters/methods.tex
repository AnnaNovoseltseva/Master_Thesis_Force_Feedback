...
\end{figure}
    ...
    \item \textbf{\textit{Force range}}. Prior studies \cite{mack_interactive_2012, prasad_modular_2003 } the retractor creates forces up to 10 N during surgeries, and majority of people perceive a force of 11N to be a solid object.  These forces correspond to       The designed device need to measure forces in that range, and if the force goes beyond that range it can be used to trigger safety alert. Max value you care to measure as well as the maximum safe value before it is overloaded and is damaged. have shown that force range applied during surgeries lies in range ($0-11 N$).
    
    \item \textbf{\textit{Sensitivity and accuracy}}. The device resolution at least $0.05 N$ and give accurate readings ($error < 0.1 N$) \cite{mack_interactive_2012}.
    
    \item \textbf{\textit{Bandwidth}}. Because the device is used for real-time haptic feedback, the minimum requirement for data acquisition speed is 1 kHz \cite{seungmoon_choi_effect_2004}.
